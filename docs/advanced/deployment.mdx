...
When you start the engine with `iii --use-default-config`, it loads the core modules plus an in-memory `modules::observability::OtelModule` configuration. Traces, metrics, and logs are enabled and stored in memory by default in this mode.
...
`iii --use-default-config` already enables the built-in observability module with in-memory exporters. Provide an explicit config only if you want to change ports, exporters, or log level.
